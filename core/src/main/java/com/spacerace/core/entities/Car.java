...
public class Car {
    ...
    private static final float FALL_DURATION = 0.75f;
    ...
    public Car(float x, float y, float rotation, CarCatalog.Entry entry) {
        ...
        this.texture = ShipTextures.get(entry.texturePath);
    }
    ...
    public void update(float delta) {
        ...
        if (hitFlashTimer > 0) hitFlashTimer = Math.max(0f, hitFlashTimer - delta);
    ...
    /** Nitro flame behind the ship (call before {@link #renderSprite}). */
    public void renderNitro(ShapeRenderer renderer) {
        if (nitroTimer <= 0 || state != PlayerState.DRIVING) return;
    ...
    public void renderSprite(SpriteBatch batch) {
        if (state == PlayerState.RESPAWNING) {
            renderRespawnGhostSprite(batch);
            return;
        }
        ...
        float alpha = (state == PlayerState.FALLING) ? Math.max(fallingScale, 0.2f) : 1f;

        float r = 1f;
        float g = 1f;
        float b = 1f;
        if (hitFlashTimer > 0f && state == PlayerState.DRIVING) {
            float flash = hitFlashTimer / HIT_FLASH_DURATION;
            r = Math.min(1f, 0.35f + flash * 0.65f);
            g = Math.min(1f, 0.35f + flash * 0.65f);
            b = Math.min(1f, 0.35f + flash * 0.65f);
        }

        batch.setColor(r, g, b, alpha);
        batch.draw(texture,
                position.x - w / 2f, position.y - h / 2f,
                w / 2f, h / 2f,
                w, h,
                1f, 1f,
                rotation - 90f,
    ...
    /** Shield aura on top of the ship (call after {@link #renderSprite}). */
    public void renderShield(ShapeRenderer renderer) {
        if (shieldTimer <= 0 || state != PlayerState.DRIVING) return;

        renderer.begin(ShapeRenderer.ShapeType.Filled);
        renderer.setColor(new Color(1f, 0.84f, 0f, 0.4f));
        float pulse = (float) Math.sin(shieldTimer * 15f) * 3f;
        renderer.circle(position.x, position.y, Math.max(WIDTH, HEIGHT) / 2f + 8f + pulse);
        renderer.end();
    }

    private void renderRespawnGhostSprite(SpriteBatch batch) {
        boolean visible = ((int) (stateTimer * 8f)) % 2 == 0;
        if (!visible) return;

        batch.setColor(1f, 1f, 1f, 0.4f);
        batch.draw(texture,
                lastSafePosition.x - WIDTH / 2f, lastSafePosition.y - HEIGHT / 2f,
                WIDTH / 2f, HEIGHT / 2f,
                WIDTH, HEIGHT,
                1f, 1f,
                lastSafeRotation - 90f,
                0, 0, texture.getWidth(), texture.getHeight(),
                false, false);
        batch.setColor(Color.WHITE);
    }
    ...
    public void applyCollisionImpulse(Vector2 pushDirection, float pushStrength, float spinKick) {
        position.add(pushDirection.x * 2f, pushDirection.y * 2f);
    ...
    public static void resolveCollision(Car a, Car b, CollisionEffectManager effects) {
        ...
        float bounceStrength = 170f + combinedSpeed * 0.45f;
        float spinAmount = 45f + combinedSpeed * 0.1f;

        if (effects != null) {
            float midX = (a.position.x + b.position.x) * 0.5f;
            float midY = (a.position.y + b.position.y) * 0.5f;
            effects.spawn(midX, midY, combinedSpeed);
